...
public class MWFActivity extends X_AD_WF_Activity implements Runnable
{
	...
	//FUNCION AGREGADA POR PABLO SALAMANCA PARA MOSTRAR LA ORGANIZACION
	public String getOrganization()
		{
			...
			//if ((keyColumns != null) && (keyColumns.length > 0))
			//	sb.append(Msg.getElement(getCtx(), keyColumns[0])).append(" ");
			int index = po.get_ColumnIndex("AD_Org_ID");
			Integer sr = null;
			if (index != -1)
				sr = (Integer)po.get_Value(index);
			else
			{
				index = po.get_ColumnIndex("AD_Org_ID");
				if (index != -1)
					sr = (Integer)po.get_Value(index);
			}
			if (sr != null)
			{
				MOrg org = MOrg.get(getCtx(), sr.intValue());
				if (org != null)
					sb.append(org.getName()).append(" ");
			}
			return sb.toString();
		}
		//FIN FUNCION AGREGADA POR PABLO SALAMANCA
}	//	MWFActivity
