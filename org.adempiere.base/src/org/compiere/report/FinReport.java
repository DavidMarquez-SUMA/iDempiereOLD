...
public class FinReport extends SvrProcess
{
	...
	protected void prepare()
	{
		...
		for (int i = 0; i < para.length; i++)
		{
			...
			else if (name.equals("SM_Marca_ID"))
				p_SM_Marca_ID = ((BigDecimal)para[i].getParameter()).intValue();
			else
				log.log(Level.SEVERE, "Unknown Parameter: " + name);
		}
		//  Optional SM_Marca_ID
		/*if (p_SM_Marca_ID != 0)
			m_parameterWhere.append(" AND AD_Org_ID IN (")
			.append("select distinct ao.ad_org_id " + 
					"from ad_org ao " + 
					"inner join ad_orginfo aoi on aoi.ad_org_id = ao.ad_org_id " + 
					"inner join ad_org ao_m on	ao_m.ad_org_id = aoi.parent_org_id " + 
					"join ad_orginfo aoi_m on aoi_m.ad_org_id = ao_m.ad_org_id " + 
					"where ao_m.ad_org_id =").append(p_SM_Marca_ID)
					.append(")");
		else{*/
		//	Optional Org
		if (p_Org_ID != 0)
	...
	//	if (p_SM_Marca_ID == 0) {
		...
		/*}else {
			sql = sql.append("INSERT INTO T_Report "
					+ "(AD_PInstance_ID, PA_ReportLine_ID, Record_ID,Fact_Acct_ID, SeqNo,LevelNo, Name,Description) "
					+ "SELECT ").append(getAD_PInstance_ID()).append(", rl.PA_ReportLine_ID, 0,0, rl.SeqNo,0, NVL(trl.Name, rl.Name) as Name, NVL(trl.Description,rl.Description) as Description "
					+ "FROM PA_ReportLine rl "
					+ "LEFT JOIN PA_ReportLine_Trl trl ON trl.PA_ReportLine_ID = rl.PA_ReportLine_ID AND trl.AD_Language = '" + Env.getAD_Language(Env.getCtx()) + "' "
					+ "WHERE rl.IsActive='Y' AND rl.PA_ReportLineSet_ID=").append(PA_ReportLineSet_ID)
					.append(" AND rl.AD_Org_ID IN (")
					.append("SELECT DISTINCT ao.AD_Org_ID " + 
							"FROM AD_Org ao " + 
							"INNER JOIN AD_OrgInfo aoi ON aoi.AD_Org_ID = ao.AD_Org_ID " + 
							"INNER JOIN AD_Org ao_m ON	ao_m.AD_Org_ID = aoi.Parent_Org_ID " + 
							"INNER JOIN AD_OrgInfo aoi_m ON aoi_m.AD_Org_ID = ao_m.AD_Org_ID " + 
							"WHERE ao_m.AD_Org_ID =").append(p_SM_Marca_ID)
							.append(")");
		}*/
