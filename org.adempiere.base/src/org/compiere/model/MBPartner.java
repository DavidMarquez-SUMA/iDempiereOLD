...
//			
			/*MODIFICACION PARA LOS CAMPOS SALDOS EN $ Y BS EN LA VENTANA TERCEROS PABLO SALAMANCA*/
			template.setSUMA_Saldo_Bs (Env.ZERO);
			template.setSUMA_Saldo_US (Env.ZERO);
	...
	public MBPartner (Properties ctx, int C_BPartner_ID, String trxName)
	{
		...
		//	setDUNS("");
			...
			/*MODIFICACION CAMPOS SALDOS EN BS Y $ EN TERCEROS PABLO SALAMANCA*/
			setSUMA_Saldo_Bs (Env.ZERO);
			setSUMA_Saldo_US (Env.ZERO);
			//
			setFirstSale(null);
	...
	public String toString ()
	{
		StringBuilder sb = new StringBuilder ("MBPartner[ID=")
			...
			//.append(",Open=").append(getSUMA_Saldo_Bs())
			//.append(",Open=").append(getSUMA_Saldo_Us())
			.append ("]");
		return sb.toString ();
	}	//	toString
	...
	/*MODIFICACION CAMPOS SALDOS EN BS Y $ EN TERCEROS PABLO SALAMANCA*/
	public void setSUMA_Saldo_Bs ()
	{
		BigDecimal SUMA_Saldo_Bs = null;
		//BigDecimal SUMA_Saldo_Bs = new java.math.BigDecimal("100");
		//AZ Goodwill -> BF2041226 : only count completed/closed docs.
		String sql = "SELECT "
			//	SO Credit Used
			+ "COALESCE((SELECT SUM(currencyBase(invoiceOpen(i.C_Invoice_ID,i.C_InvoicePaySchedule_ID),i.C_Currency_ID,i.DateInvoiced, i.AD_Client_ID,i.AD_Org_ID)) FROM C_Invoice_v i "
				+ "WHERE i.C_BPartner_ID=bp.C_BPartner_ID AND i.IsSOTrx='Y' AND i.IsPaid='N' AND i.C_Currency_ID=205 AND i.DocStatus IN ('CO','CL')),0), "
			//	Balance (incl. unallocated payments)
			+ "COALESCE((SELECT SUM(currencyBase(invoiceOpen(i.C_Invoice_ID,i.C_InvoicePaySchedule_ID),i.C_Currency_ID,i.DateInvoiced, i.AD_Client_ID,i.AD_Org_ID)*i.MultiplierAP) FROM C_Invoice_v i "
				+ "WHERE i.C_BPartner_ID=bp.C_BPartner_ID AND i.IsPaid='N' AND i.C_Currency_ID=205 AND i.DocStatus IN ('CO','CL')),0) - "
			+ "COALESCE((SELECT SUM(currencyBase(Paymentavailable(p.C_Payment_ID),p.C_Currency_ID,p.DateTrx,p.AD_Client_ID,p.AD_Org_ID)) FROM C_Payment_v p "
				+ "WHERE p.C_BPartner_ID=bp.C_BPartner_ID AND p.IsAllocated='N' AND p.C_Currency_ID=205"
				+ " AND p.C_Charge_ID IS NULL AND p.DocStatus IN ('CO','CL')),0) "
			+ "FROM C_BPartner bp "
			+ "WHERE C_BPartner_ID=?";		
		PreparedStatement pstmt = null;
		...
		try
		{
			...
			if (rs.next ())
			{
				...
				//Msg.getMsg(Env.getCtx(), rs.getBigDecimal(1).toString());
				//log.log(Level.SEVERE, rs.getBigDecimal(1).toString());
				SUMA_Saldo_Bs = rs.getBigDecimal(2);
			}
		}
		catch (Exception e)
		{
		...
		if (SUMA_Saldo_Bs != null)
			super.setSUMA_Saldo_Bs(SUMA_Saldo_Bs);
		//SUMA_Saldo_Bs = getBigDecimal(100);

	}	//	setSUMA_Saldo_Bs
	
	public void setSUMA_Saldo_US ()
	{

		BigDecimal SUMA_Saldo_US = null;
		//AZ Goodwill -> BF2041226 : only count completed/closed docs.
		String sql = "SELECT COALESCE(saldo_bs,0) FROM SM_RV_Saldo_Tercero rv WHERE rv.C_BPartner_ID = ?";		
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		try
		{
			...
			if (rs.next ())
			{
				SUMA_Saldo_US = rs.getBigDecimal(1);
			}
		}
		catch (Exception e)
		{
		...
		if (SUMA_Saldo_US != null)
			super.setSUMA_Saldo_US(SUMA_Saldo_US);
	}	//	setSUMA_Saldo_Us
	/*FIN MODIFICACION SALDOS EN BS Y $ EN TERCEROS PABLO SALAMANCA*/
