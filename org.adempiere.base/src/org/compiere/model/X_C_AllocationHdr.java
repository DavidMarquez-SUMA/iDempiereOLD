...
// DR
	...
	{
		//INICIO DE MODIFICACION LS && PS
		Calendar TT = Calendar.getInstance();
		Timestamp DateAcct = (Timestamp)get_Value(COLUMNNAME_DateAcct);
		TT.setTimeInMillis(DateAcct.getTime());
		TT.set(Calendar.HOUR_OF_DAY, 23);
		TT.set(Calendar.MINUTE, 59);
		TT.set(Calendar.SECOND, 59);
		TT.set(Calendar.MILLISECOND, 0);		
		DateAcct = new Timestamp (TT.getTimeInMillis());
		Msg.getMsg(Env.getCtx(), DateAcct.toString());
		return DateAcct;
		// FIN DE MODIFICACION
		//return (Timestamp)get_Value(COLUMNNAME_DateAcct); -- ORIGINAL DEL SISTEMA (COMENTADO) POR LS & PS
	}
