...
public class MAcctSchema extends X_C_AcctSchema
{
	...
	private MInterOrgAcctSM			m_gl2 = null;
	/** GL Info				*/
	...
	public MInterOrgAcctSM getAcctSchemaGL2(int AD_Org_ID_SM)
	{
		m_gl2=null;
		if (m_gl2 == null)
			m_gl2 = geto(getCtx(), getC_AcctSchema_ID(), AD_Org_ID_SM);
		//if (m_gl2 == null)
			//throw new IllegalStateException("No es GL Definition for C_AcctSchema_ID=" + getC_AcctSchema_ID());
		return m_gl2;

	}	//	getAcctSchemaGL
	public MInterOrgAcctSM geto (Properties ctx, int C_AcctSchema_ID, int AD_Org_ID_SM)
	{
		ArrayList<Object> params = new ArrayList<Object>();
		StringBuilder whereClause =  new StringBuilder("C_AcctSchema_ID=?")		//	#1
				.append(" AND AD_Org_ID=?");		
					  //whereClause = "C_AcctSchema_ID=?";
		params.add(C_AcctSchema_ID);
		params.add(AD_Org_ID_SM);
		AD_Org_ID_SM=0;
		//java.lang.System.out.println("final2 ORG: "+whereClause.toString()+AD_Org_ID_SM);
		return new Query(ctx,I_C_InterOrg_Acct_SM.Table_Name,whereClause.toString(),null)
				.setParameters(params)
	 ...
	 *  @return Account
	 */
	public MAccount getDueTo_Acct(String segment, int AD_Org_ID_SM)
	{
		m_DueTo_Acct=null;
		//java.lang.System.out.println("Org: "+getAD_Org_ID()+" -- "+segment);
		int C_ValidCombination_ID;
		getAcctSchemaGL2(AD_Org_ID_SM);
		//java.lang.System.out.println("ORG: "+AD_Org_ID_SM+" --- "+m_gl2+"   ----> getDueToAcct");
		//java.lang.System.out.println(m_gl2);
		if (m_DueTo_Acct != null)
			return m_DueTo_Acct;
		if (m_gl2 == null) {
			getAcctSchemaGL();
			C_ValidCombination_ID = m_gl.getIntercompanyDueTo_Acct();
		}else {
			 C_ValidCombination_ID = m_gl2.getIntercompanyDueTo_Acct();
			 
		}
		m_DueTo_Acct = MAccount.get(getCtx(), C_ValidCombination_ID);
		C_ValidCombination_ID=0;
	//	java.lang.System.out.println(m_DueTo_Acct);
		return m_DueTo_Acct;
	}	//	getDueTo_Acct
	 ...
	 *  @return Account
	 */
	public MAccount getDueFrom_Acct(String segment, int AD_Org_ID_SM)
	{
		m_DueFrom_Acct=null;
		int C_ValidCombination_ID;
		getAcctSchemaGL2(AD_Org_ID_SM);
		//java.lang.System.out.println("ORG: "+AD_Org_ID_SM+" --- "+m_gl2+"   ----> getDueFromAcct");
		if (m_DueFrom_Acct != null)
			return m_DueFrom_Acct;
		//if (m_gl == null)
		if (m_gl2 == null) {
			getAcctSchemaGL();
			C_ValidCombination_ID = m_gl.getIntercompanyDueFrom_Acct();
		}else {
			C_ValidCombination_ID = m_gl2.getIntercompanyDueFrom_Acct();
		}
