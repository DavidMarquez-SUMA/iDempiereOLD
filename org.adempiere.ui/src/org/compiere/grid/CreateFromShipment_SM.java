 ...
 */
public abstract class CreateFromShipment_SM extends CreateFrom 
{
	/**  Loaded Invoice             */
	 ...
	 *  @param mTab MTab
	 */
	public CreateFromShipment_SM(GridTab mTab)
	{
		super(mTab);
...
//                + "-COALESCE((SELECT SUM(MovementQty) FROM M_InOutLine iol JOIN M_InOut io ON iol.M_InOut_ID=io.M_InOut_ID WHERE l.C_OrderLine_ID=iol.C_OrderLine_ID),0),"	//	1 /*MODIFICACION*/
				+ "-COALESCE((SELECT SUM(MovementQty) FROM M_InOutLine iol JOIN M_InOut io ON iol.M_InOut_ID=io.M_InOut_ID WHERE l.C_OrderLine_ID=iol.C_OrderLine_ID AND CASE WHEN io.MovementType ='V+' THEN io.Processed='N' ELSE io.Processed IN ('N','Y') END),0),"	//	1
		...
		try
		{
			...
			while (rs.next())
			{
				...
				if (qtyOrdered.compareTo(new BigDecimal(0))==1) {
				/*MODIFICACION CAOS [*=*]*/
