...
public abstract class CreateFromShipment extends CreateFrom 
{
	...
	protected Vector<Vector<Object>> getOrderData (int C_Order_ID, boolean forInvoice)
	{
		...
		StringBuilder sql = new StringBuilder("SELECT "
                ...
                + "-COALESCE((SELECT SUM(MovementQty) FROM M_InOutLine iol JOIN M_InOut io ON iol.M_InOut_ID=io.M_InOut_ID WHERE l.C_OrderLine_ID=iol.C_OrderLine_ID AND CASE WHEN io.MovementType ='V+' THEN io.Processed='N' ELSE io.Processed IN ('N','Y') END),0),"	//	1 /*MODIFICACION*/
